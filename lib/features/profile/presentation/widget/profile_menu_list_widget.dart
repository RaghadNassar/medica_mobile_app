...
import 'package:raghad_pro/core/constanse/app_route.dart'; // تأكدي من مسار الـ routes الصحيح لديكِ
import 'package:raghad_pro/core/constanse/string_manager.dart';
...
class ProfileMenuList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final List<Map<String, dynamic>> menuItems = [
      {'icon': Icons.person_outline, 'title': StringManager.editProfile,'route': AppRoutes.editProfile},
      {'icon': Icons.medical_services_outlined, 'title': StringManager.medicalRecord, 'route': AppRoutes.medicalHistory},
      {'icon': Icons.settings_outlined, 'title': StringManager.settings, 'route': AppRoutes.settings},
     
      {'icon': Icons.notifications, 'title': StringManager.notification, 'isNotification': true, 'route': AppRoutes.notifications},
    ];
    ...
    return CustomMenuContainer(
      children: List.generate(menuItems.length, (index) {
        ...
        Widget buildNormalMenuItem() {
          ...
          // نقوم بملء الحقول بالبيانات الحالية فوراً قبل الانتقال
               Get.find<ProfileController>().fillControllersWithCurrentData();
        }
                // سيقوم بالتوجيه إلى الصفحة الكاملة سواء كانت إعدادات أو إشعارات أو تاريخ طبي
                Get.toNamed(item['route']);
              }
            },
          );
...
} // 👈 إغلاق كلاس _CustomDivider بقوس واحد فقط
void _showNotificationsBottomSheet(BuildContext context, bool isDark, NotificationLogic _notificationService) {
    Get.bottomSheet(
      Container(
                      ...
                      ? TextButton.icon(
                          ...
                          onPressed: () {
                            Get.defaultDialog(
                              ...
                              onConfirm: () {
                                _notificationService.clearAllNotificationsFromServer();
                                Get.back();
                              },
                            );
                          },
                          label: const Text("مسح الكل", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13)),
                        )
              ...
              child: Obx(() {
                ...
                return ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    AppNotification notif = _notificationService.serverNotifications[index];
                    
                    bool isShiftSwap = notif.type == 'shift_swap';
                    Color dynamicColor = isShiftSwap ? Colors.orangeAccent : AppColors.primaryTeal;
                    ...
                    return Dismissible(
                      ...
                      background: Container(
                        ...
                        child: const Row(
                          ...
                          children: [
                            Text("حذف التنبيه", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
                            Spacer(),
                      ...
                      onDismissed: (direction) {
                        _notificationService.removeSingleNotification(notif.uuid);
                      },
                      child: GestureDetector(
                        onTap: () {
                          if (!notif.isRead) {
                            _notificationService.markNotificationAsRead(notif.uuid);
                          }
                        },
                        child: Container(
                          margin: const EdgeInsets.only(bottom: 14),
                          ...
                          child: ClipRRect(
                            ...
                            child: Stack(
                              children: [
                                Positioned(
                                  top: 0, right: 0, bottom: 0, width: 5,
                                  ...
                                  child: Row(
                                    ...
                                    children: [
                                      ...
                                      Container(
                                        ...
                                        child: Icon(
                                          isShiftSwap ? Icons.swap_horiz_rounded : Icons.notifications_none_rounded, 
                                          color: dynamicColor, 
                                          size: 22
                                        ),
                                      ),
      ...
      child: Column(
        ...
        children: [
          Stack(
            alignment: Alignment.center,
            children: [
              Container(width: 110, height: 110, decoration: BoxDecoration(color: AppColors.primaryTeal.withOpacity(0.06), shape: BoxShape.circle)),
              Container(width: 85, height: 85, decoration: BoxDecoration(color: AppColors.primaryTeal.withOpacity(0.12), shape: BoxShape.circle)),
              const Icon(Icons.notifications_none_rounded, size: 52, color: AppColors.primaryTeal),
            ],
          ),
          const SizedBox(height: 24),
          Text(
            "صندوق الوارد فارغ!",
            style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: isDark ? Colors.white : AppColors.darkAppBar),
          ),
