...
//     );
//   }
//   // === أضيفي هذا القسم داخل كلاس AuthLogic ===

// var currentStep = 0.obs; // 0 تعني الخطوة الأولى، 1 تعني الخطوة الثانية
// final GlobalKey<FormState> formKeyStepOne = GlobalKey<FormState>();
// final GlobalKey<FormState> formKeyStepTwo = GlobalKey<FormState>();

// // دالة الانتقال تصبح بسيطة جداً وتعتمد على الفاليديشن الخاص بالواجهة للخطوة الأولى
// void nextStep() {
...
//   }
// }
// // ميكانيكية التحقق من الخطوة الأولى فقط (الاسم، اللقب، الإيميل، الباسوردات)
// bool validateStepOne() {
//   // نقوم بالتحقق من الحقول الأساسية يدوياً عبر الـ Validators التي استخدمتِها في الواجهة
//   if (Validator.validateRequiredField(nameController.text, StringManager.username) != null ||
//       Validator.validateRequiredField(nickNameController.text, StringManager.nickname) != null ||
//       Validator.validateEmail(emailUpController.text) != null ||
//       Validator.validatePassword(passwordUPController.text) != null ||
//       Validator.validateConfirmPassword(confirmpasswordUPController.text, passwordUPController.text) != null) {
    
//     // أظهري سناك بار ينبه المستخدم بوجود حقول خاطئة بالخطوة الحالية
//     AlertHelper.showSnackbar(
//       title: "تنبيه",
//       message: "يرجى ملء بيانات الخطوة الأولى بشكل صحيح",
//       type: AlertType.warning,
//     );
//     return false;
//   }
